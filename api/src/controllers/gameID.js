...
const getGameApi = async (idGame) => {
    const apiUrl = await axios.get(`https://api.rawg.io/api/games/${idGame}?key=${key}`);
...
const getGameDB = async (idGame) => {
    ...
    const data = gameDBArray.map(({ id, name, description, released, img, rating, platforms, genres }) => (
        {
            id,
            name,
            description,
